...
class Checklist(models.Model):
    ...
    action_id = fields.Many2one(
        "ir.actions.server", "Action", help="Run when checklist becomes complete"
    )
    act_window_ids = fields.Many2many(
        "ir.actions.act_window",
        "checklist_act_window_rel",
        "act_window_id",
        "checklist_id",
        "Menus",
    )
    view_ids = fields.Many2many(
        "ir.ui.view", "checklist_view_rel", "view_id", "checklist_id", "Views"
    )
    ...
    def _check_unique_checklist_per_model(self):
        for checklist in self:
            ...
            domain = [
                ("model_id", "=", checklist.model_id.id),
                ("id", "!=", checklist.id),
            ]
            ...
            if not checklist._context.get("do_not_compute_progress_rates"):
                ...
                for record in records.with_context(active_test=True):
                    ...
                    if vals:
                        ...
                        if (
                            checklist.action_id
                            and old_progress_rate
                            != record.x_checklist_progress_rate
                            == 100.0
                        ):
                            ctx = {
                                "active_id": record.id,
    ...
    def _get_record_vals(self, record):
        def compute_progress_rate(task_insts):
            ...
            return round(
                100.0
                * (
                    len(task_insts.filtered(lambda inst: inst.complete))
                    / len(task_insts)
                ),
                0,
            )
        ...
        for task_inst in task_insts:
            task_inst.complete = bool(
                record.filtered_domain(safe_eval(task_inst.task_id.complete_domain))
            )
        ...
        if self.active_field:
            ...
            if task_insts:
                vals["x_checklist_progress_rate_mandatory"] = compute_progress_rate(
                    task_insts
                )
    ...
    def _register_hook(self):
        def make_create():
            ...
            def create(self, vals_list, **kw):
                ...
                for record in records:
                    ...
                    record._get_checklist_task_instances()
                return records
        ...
        def make_fields_view_get():
            ...
            def get_view(self, view_id=None, view_type="form", **options):
                ...
                self.env["checklist"].with_context(
                    act_window_id=action_id
                )._complete_view(self._name, view_type, res)
                return res
        ...
        for checklist in self.search(
            [("name", "!=", False), ("model_id", "!=", False)]
        ):
    ...
    def _complete_view(self, model, view_type, res):
        ...
        checklist = self.browse(checklist_id).read(
            ["name", "act_window_ids", "view_ids"], load="_classic_write"
        )[0]
        if (
            checklist["act_window_ids"]
            and self._context.get("act_window_id")
            and self._context["act_window_id"] not in checklist["act_window_ids"]
        ):
            return
        if checklist["view_ids"] and res.get("id") not in checklist["view_ids"]:
            return
        if not self._context.get("no_checklist") and view_type in ("list", "form"):
            if view_type == "list" and model == res.get("model"):
                arch = res["arch"]
                if (
                    etree.XML(res["arch"]).find(
                    ...
                    res["arch"] = "".join(
                        [
                            arch[:idx],
                            """<field name="x_checklist_progress_rate"
                            string="%s" readonly="1" widget="progressbar"
                        />"""
                            % _("Progress Rate"),
                            arch[idx:],
                        ]
                ...
                if root.find(".//button[@name='open_checklist']") is None:
                    ...
                    button_box.insert(
                        ...
                        <button class="oe_stat_button" type="object"
                            name="open_checklist"
                            invisible="not x_checklist_task_instance_ids">
                            <field name="x_checklist_progress_rate"
                                class="o_button_icon" widget="percentpie"
                                string="%s" nolabel="1"/>
                            <field name="x_checklist_task_instance_ids"
                                invisible="1"/>
                        </button>
                    """
                            % checklist["name"]
    ...
    def create(self, vals):
        self = self
        checklist = super(
            Checklist, self.with_context(do_not_compute_progress_rates=True)
        ).create(vals)
        self.env.registry.clear_cache()
        checklist.with_context(
            do_not_compute_progress_rates=False
        )._compute_progress_rates()
        return checklist
    ...
    def write(self, vals):
        ...
        if "model_id" in vals:
            checklists_to_recompute.mapped("task_ids").with_context(
                do_not_compute_progress_rates=True
            )._manage_task_instances()
