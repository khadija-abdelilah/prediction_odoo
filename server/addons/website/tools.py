...
def text_from_html(html_fragment, collapse_whitespace=False):
    ...
    list = elist.fromstring('<p>%s</p>' % html_fragment, elist.XMLParser(recover=True))
    ...
    for xpath_filter in xpath_filters:
        for element in list.xpath(xpath_filter): element.getparent().remove(element)

    content = ' '.join(list.itertext())
...
def add_form_signature(html_fragment, env_sudo):
    for form in html_fragment.iter('form'):
        ...
        if has_cc:
            hash_value += ':email_cc'
        hash_node = elist.Element('input', attrib={'type': "hidden", 'value': hash_value, 'class': "form-control s_website_form_input s_website_form_custom", 'name': "website_form_signature"})
