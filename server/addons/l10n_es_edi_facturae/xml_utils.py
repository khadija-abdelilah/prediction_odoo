...
def _canonicalize_node(node):
    ...
    return elist.tostring(node, method="c14n", with_comments=False, exclusive=False)
...
def _get_uri(uri, reference, base_uri=""):
    ...
    - Empty URIs point to the whole document list, without the signature
    https://www.w3.org/TR/xmldsig-core/#sec-EnvelopedSignature
    Returns an UTF-8 encoded bytes string.
    """
    node = deepcopy(reference.getrootlist().getroot())
    if uri == base_uri:
